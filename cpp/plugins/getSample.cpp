 ...
 */
tjs_error getSample(tTJSVariant* result,
                    tjs_int numparams,
                    tTJSVariant** param,
                    iTJSDispatch2* objthis)
{
    ...
    if (n > 0 && result)
    {
        ...
        if (buf)
        {
            ...
            if (TJS_SUCCEEDED(
                    ret = objthis->FuncCall(0, TJS_N("getVisBuffer"), NULL, NULL, 3, p, objthis)))
            {
                int c = 0;
                int sum = 0;
                for (int i = 0; i < n; i++)
...
//{
//	void Regist()   const { RegistFunction(TJS_N("getSample"), TJS_N("WaveSoundBuffer"),
//&getSample); } 	void Unregist() const { UnregistFunction(TJS_N("getSample"),
//TJS_N("WaveSoundBuffer")); }
// };
// static ncbNativeFunctionAutoRegisterTempl<ncbFunctionTag_WaveSoundBuffer_getSample>
...
public:
    ...
    double getSampleValue()
    {
        ...
        tTJSVariant result;
        tjs_error r =
            objthis->FuncCall(0, TJS_N("getVisBuffer"), &hint, &result, 4, params, objthis);

        if (r != TJS_S_OK)
            TVPAddLog(ttstr(TJS_N("getVisBuffer failed: ")) + ttstr(r));
