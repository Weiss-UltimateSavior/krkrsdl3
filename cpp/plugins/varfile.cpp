...
protected:
    ...
    // 読み込み用メモリ領域取得
    const tjs_uint8* getBase() { return isFile(value) ? value.AsOctetNoAddRef()->GetData() : NULL; }
...
public:
    ...
    virtual bool CheckExistentStorage(const ttstr& name) { return isFile(getFile(name)); }
...
protected:
        ...
        while (path.length() > 0)
        {
            ...
            else
            {
                ...
                if (((o.IsInstanceOf(0, NULL, NULL, TJS_N("Array"), NULL) == TJS_S_TRUE &&
                      TJS_SUCCEEDED(o.PropGetByNum(0, (tjs_int)TJSStringToInteger(member.c_str()),
                                                   &value, NULL))) ||
                     (TJS_SUCCEEDED(o.PropGet(0, member.c_str(), NULL, &value, NULL)))) &&
                    isDirectory(value))
                {
                    base = value;
                    path = ttstr(q + 1);
    ...
    tTJSVariant getFile(const ttstr& name)
    {
        ...
        if (isDirectory(base) && fname.length() > 0)
        {
            ...
            tTJSVariant value;
            if (TJS_SUCCEEDED(
                    base.AsObjectClosureNoAddRef().PropGet(0, fname.c_str(), NULL, &value, NULL)))
            {
                base = value;
            }
            else
...
static void PreRegistCallback()
{
    if (var == NULL)
    {
        ...
        if (global)
        {
            ...
            global->PropSet(TJS_MEMBERENSURE, TJS_N("varfileLoaded"), NULL, &val, global);
            global->Release();
        }
    }
}
