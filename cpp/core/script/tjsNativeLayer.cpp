...
static bool IsGPU() {
    static bool isGPU =
        !TVPIsSoftwareRenderManager() && !GameSetting::ogl_accurate_render;
    return isGPU;
}
...
//---------------------------------------------------------------------------
bool tTJSNI_BaseLayer::GetHasImage() const { return MainImage != NULL; }
//---------------------------------------------------------------------------
...
tTJSNI_BaseLayer *tTJSNI_BaseLayer::GetMostFrontChildAt(tjs_int x, tjs_int y,
    ...
    return Manager->GetMostFrontChildAt(x, y, exclude_self ? this : NULL,
                                        get_disabled);
}
//---------------------------------------------------------------------------
    ...
    if(Opacity < 255 || (InTransition && TransWithChildren)) {
        ...
        if(GetVisibleChildrenCount() == 0) {
            ...
            if(MainImage != NULL) {
                ...
                // UpdateExcludeRect.bottom >= rect.bottom && 					rect.left >=
...
void tTJSNI_BaseLayer::InternalDrawNoCache_CPU(tTVPDrawable *target,
    ...
    if(GetVisibleChildrenCount() == 0) {
        ...
        DirectTransferToParent =
            true; // this flag is used only when MainImage == NULL
...
tTVPBaseTexture *tTJSNI_BaseLayer::GetDrawTargetBitmap(const tTVPRect &rect,
    ...
    if(DisplayType == ltBinder ||
       (MainImage == NULL && DirectTransferToParent)) {
        tTVPRect _rect(rect);
