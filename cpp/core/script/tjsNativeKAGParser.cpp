...
const tjs_char* TVPKAGCannotOmmitFirstLabelName = TJS_N("Can not ommit first label name in scenario.");
...
const tjs_char* TVPKAGSyntaxError = TJS_N("Syntax error.'[' match to ']', \" match to \", 'macro' march to 'endmacro'. Notice space and newline.");
const tjs_char* TVPKAGCallStackUnderflow = TJS_N("'return' is not matched to any 'call' ( 'return' is unexpected )");
const tjs_char* TVPKAGReturnLostSync = TJS_N("Lost return position due to the scenario file changed.");
...
iTJSDispatch2* tTJSNI_KAGParser::Store()
{
	...
	try
	{
		...
		// create and assign macro dictionary
		{
			...
			dsp->Release();
			dic->PropSet(TJS_MEMBERENSURE, TJS_N("macros"), NULL,
				&tmp, dic);
		...
		// create and assign macro arguments
		{
			...
			tTJSVariant tmp(dsp, dsp);
			dsp->Release();
			dic->PropSet(TJS_MEMBERENSURE, TJS_N("macroArgs"), NULL,
				&tmp, dic);
		...
		// create call stack array and copy call stack status
		{
			...
			tTJSVariant tmp(dsp, dsp);
			dsp->Release();
			dic->PropSet(TJS_MEMBERENSURE, TJS_N("callStack"), NULL,
				&tmp, dic);
			...
			for (i = CallStack.begin(); i != CallStack.end(); i++)
			{
				...
				tTJSVariant val;
				val = i->Storage;
				dic->PropSet(TJS_MEMBERENSURE, TJS_N("storage"), NULL,
					&val, dic);
				val = i->Label;
				dic->PropSet(TJS_MEMBERENSURE, TJS_N("label"), NULL,
					&val, dic);
				val = i->Offset;
				dic->PropSet(TJS_MEMBERENSURE, TJS_N("offset"), NULL,
					&val, dic);
				val = i->OrgLineStr;
				dic->PropSet(TJS_MEMBERENSURE, TJS_N("orgLineStr"), NULL,
					&val, dic);
				val = i->LineBuffer;
				dic->PropSet(TJS_MEMBERENSURE, TJS_N("lineBuffer"), NULL,
					&val, dic);
				val = i->Pos;
				dic->PropSet(TJS_MEMBERENSURE, TJS_N("pos"), NULL,
					&val, dic);
				...
				dic->PropSet(TJS_MEMBERENSURE, TJS_N("macroArgStackDepth"), NULL,
					&val, dic);
				val = i->ExcludeLevel;
				dic->PropSet(TJS_MEMBERENSURE, TJS_N("ExcludeLevel"), NULL,
					&val, dic);
				val = (tjs_int)i->IfLevel;
				dic->PropSet(TJS_MEMBERENSURE, TJS_N("IfLevel"), NULL,
					&val, dic);
		...
		val = StorageName;
		dic->PropSet(TJS_MEMBERENSURE, TJS_N("storageName"), NULL,
			&val, dic);
		val = StorageShortName;
		dic->PropSet(TJS_MEMBERENSURE, TJS_N("storageShortName"), NULL,
			&val, dic);
		...
		val = CurLine;
		dic->PropSet(TJS_MEMBERENSURE, TJS_N("curLine"), NULL,
			&val, dic);
		val = CurPos;
		dic->PropSet(TJS_MEMBERENSURE, TJS_N("curPos"), NULL,
			&val, dic);
		...
		val = CurLabel;
		dic->PropSet(TJS_MEMBERENSURE, TJS_N("curLabel"), NULL,
			&val, dic);
...
void tTJSNI_KAGParser::Restore(iTJSDispatch2* dic)
{
...

	{
		...
		if (val.Type() != tvtVoid)
		{
			...
			val.Clear();
			dic->PropGet(0, TJS_N("macroArgStackDepth"), NULL,
				&val, dic);
		...
		val.Clear();
		dic->PropGet(0, TJS_N("storageName"), NULL,
			&val, dic);
		if (val.Type() != tvtVoid) StorageName = val;
		val.Clear();
		dic->PropGet(0, TJS_N("storageShortName"), NULL,
			&val, dic);
		if (val.Type() != tvtVoid) StorageShortName = val;
		val.Clear();
		dic->PropGet(0, TJS_N("curLabel"), NULL,
			&val, dic);
		...
		val.Clear();
		dic->PropGet(0, TJS_N("ExcludeLevel"), NULL,
			&val, dic);
		if (val.Type() != tvtVoid) ExcludeLevel = (tjs_int)val;
		val.Clear();
		dic->PropGet(0, TJS_N("IfLevel"), NULL,
			&val, dic);
		...
		val.Clear();
		dic->PropGet(0, TJS_N("macroArgStackBase"), NULL,
			&val, dic);
...
void tTJSNI_KAGParser::PopCallStack(const ttstr& storage, const ttstr& label)
{
	...
	else
	{
		...
		{
			ttstr label;
			if (data.Label.IsEmpty()) label = TJS_N("(start)"); else label = data.Label;
	...
	while (true)
	{
		...
		if (!IgnoreCR)
		{
			...
			if (CurLineStr[CurPos] == 0)
			{
				...
				if (RecordingMacro) RecordingMacroStr += TJS_N("[r eol=true]");
				CurLine++;
				CurPos = 0;
				LineBufferUsing = false;
		...
		else
		{
				...
				CurLineStr[CurPos + 1] == TJS_N('['))
			{
				...
				else
				{
					...
					DicObj->PropSetByVS(TJS_MEMBERENSURE,
						__tag_name.AsVariantStringNoAddRef(), &r_val, DicObj);
					if (RecordingMacro) RecordingMacroStr += TJS_N("[r]");
				}

				if (chLen == 1 && CurLineStr[CurPos] == TJS_N('[')) CurPos++;
				CurPos += chLen;
		...
		if (!tag_checker_init)
		{
			...
			special_tags_hash.Add(
				ttstr(TJS_N("if")), (tjs_int)tag_if);
			special_tags_hash.Add(
				ttstr(TJS_N("ignore")), (tjs_int)tag_ignore);
			special_tags_hash.Add(
				ttstr(TJS_N("endif")), (tjs_int)tag_endif);
			special_tags_hash.Add(
				ttstr(TJS_N("endignore")), (tjs_int)tag_endignore);
			special_tags_hash.Add(
				ttstr(TJS_N("else")), (tjs_int)tag_else);
			special_tags_hash.Add(
				ttstr(TJS_N("elsif")), (tjs_int)tag_elsif);
			special_tags_hash.Add(
				ttstr(TJS_N("emb")), (tjs_int)tag_emb);
			special_tags_hash.Add(
				ttstr(TJS_N("macro")), (tjs_int)tag_macro);
			special_tags_hash.Add(
				ttstr(TJS_N("endmacro")), (tjs_int)tag_endmacro);
			special_tags_hash.Add(
				ttstr(TJS_N("macropop")), (tjs_int)tag_macropop);
			special_tags_hash.Add(
				ttstr(TJS_N("erasemacro")), (tjs_int)tag_erasemacro);
			special_tags_hash.Add(
				ttstr(TJS_N("jump")), (tjs_int)tag_jump);
			special_tags_hash.Add(
				ttstr(TJS_N("call")), (tjs_int)tag_call);
			special_tags_hash.Add(
				ttstr(TJS_N("return")), (tjs_int)tag_return);
		}
...
#define TVP_KAG_STEP_NEXT \
						...
						{ \
							...
							{ \
								TVPAddLog(StorageShortName + TJS_N(" : ignore after multi-line tag : ") + (&CurLineStr[CurPos+1])); \
							} \
							CurLine = multiLine+1; \
							CurPos = 0; \
							LineBufferUsing = false; \
		...
		while (true)
		{
			...
			else
			{
				...
				if (valueend != valuestart)
				{
					...
					if (!entity && *vp == TJS_N('&')) entity = true, vp++;
					if (!macroarg && *vp == TJS_N('%')) macroarg = true, vp++;
...
iTJSDispatch2* TVPCreateNativeClass_KAGParser() {
	tTJSNativeClassForPlugin* classobj = TJSCreateNativeClassForPlugin(TJS_N("KAGParser"), Create_NI_KAGParser);
