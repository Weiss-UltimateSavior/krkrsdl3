...
class tFreeTypeFace
{
    ...
    tjs_uint (*UnicodeToLocalChar)(tjs_wchar in); //!< SJISなどをUnicodeに変換する関数
    tjs_wchar (*LocalCharToUnicode)(tjs_uint in); //!< UnicodeをSJISなどに変換する関数
...
public:
	...
	tjs_wchar GetCharcodeFromGlyphIndex(tjs_uint index);
	...
	tjs_wchar GetDefaultChar() const {
		return Face->GetDefaultChar();
	}
	tjs_wchar GetFirstChar() {
		FT_UInt gindex;
		return static_cast<tjs_wchar>( FT_Get_First_Char( FTFace, &gindex ) );
	}
	...
	tTVPCharacterData * GetGlyphFromCharcode(tjs_wchar code);
	bool GetGlyphRectFromCharcode(struct tTVPRect& rt, tjs_wchar code, tjs_int& advancex, tjs_int& advancey );
	bool GetGlyphMetricsFromCharcode(tjs_wchar code, tGlyphMetrics & metrics);
	bool GetGlyphSizeFromCharcode(tjs_wchar code, tGlyphMetrics & metrics);

	const FT_Outline* GetOulineData(tjs_wchar code, float &w, float &h);
	tBaseFreeTypeFace *GetBaseFace() { return Face; }

private:
	bool LoadGlyphSlotFromCharcode(tjs_wchar code);
};
//---------------------------------------------------------------------------
