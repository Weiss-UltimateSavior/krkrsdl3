...
//---- ユーザーインターフェース関連 
	...
	//! @param		cy		触れている高さ 
	//! @param		flag	タッチ状態フラグ 
	virtual void OnTouchScaling( tjs_real startdist, tjs_real curdist, tjs_real cx, tjs_real cy, tjs_int flag ) = 0;
	...
	//! @param		cy		触れている高さ 
	//! @param		flag	タッチ状態フラグ 
	virtual void OnTouchRotate( tjs_real startangle, tjs_real curangle, tjs_real dist, tjs_real cx, tjs_real cy, tjs_int flag ) = 0;
	...
	//! @param		width		画面幅 
	//! @param		height		画面高さ 
	virtual void OnDisplayRotate( tjs_int orientation, tjs_int rotate, tjs_int bpp, tjs_int width, tjs_int height ) = 0;
	...
	virtual void SetHintText(iTVPLayerManager * manager, iTJSDispatch2* sender, const ttstr & text) = 0;
	...
	//!				するインターフェースは今のところない。 
	//! @note		座標はプライマリレイヤ上の座標なので、必要ならば変換を行う。 
	virtual void SetAttentionPoint(iTVPLayerManager * manager, tTJSNI_BaseLayer *layer,
...
//---- デバッグ支援 
	...
	virtual bool SwitchToFullScreen( int window, tjs_uint w, tjs_uint h, tjs_uint bpp, tjs_uint color, bool changeresolution ) = 0;
	...
	//! @param		color		16bpp の時 565 か 555を指定 
	virtual void RevertFromFullScreen(int window, tjs_uint w, tjs_uint h, tjs_uint bpp, tjs_uint color) = 0;
