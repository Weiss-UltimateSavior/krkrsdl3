        ...
        if(vc->vendor)
        {
            ...
            if(vorbis_date)
            {
                ...
                if(date < WARN_OLD_VORBIS_DATE)
                {
                    ...
                    TVPAddLog(TJS_N("wuvorbis: warning: The Vorbis stream \"") + ttstr(url) + TJS_N("\" seems to be generated by old Vorbis encoder [") + 
                        ttstr(vc->vendor) + TJS_N("]. Check that your encoder is new enough, unless you prefer the old encoder. The old stream can still be decoded, but ") + ttstr(WARN_PREFER_ENCODER_VERSION) );
                }
            }
        }
    }
