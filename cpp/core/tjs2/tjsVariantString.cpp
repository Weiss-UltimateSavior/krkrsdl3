...
{
    char* ret =
        (char*)malloc((len = (len + TJSVS_ALLOC_INC_SIZE_S)) * sizeof(tjs_char) + sizeof(size_t));
    if (!ret)
    ...
    if (!TJSStringHeapList->empty())
    {
        ...
        for (c = TJSStringHeapList->end() - 1; c >= TJSStringHeapList->begin(); c--)
        {
            ...
            for (tjs_int i = 0; i < HEAP_CAPACITY_INC; i++)
            {
                ...
                {
                    // using cell
                    tTJSNarrowStringHolder narrow(h[i].LongString ? h[i].LongString
                                                                  : h[i].ShortString);
...
#define TJS_CHECK_FREE_BLOCK_MAX 50
    ...
    { // thread-protected
        ...
        if (TJSStringHeapFreeCellListPointer > TJS_SORT_STR_CELL_MAX)
        {
            ...
            start = TJSStringHeapFreeCellList + TJSStringHeapFreeCellListPointer -
                    TJS_SORT_STR_CELL_MAX;
...
//---------------------------------------------------------------------------
tjs_int tTJSVariantString::GetCharLength() const
{
    return utf8_chars_len(*this);
}
//---------------------------------------------------------------------------
...
public:
    tTVPVariantStringHolder() { String = TJSAllocVariantString(TJS_N("This is a dummy.")); }
...
#define check_alloc \
    ...
    width_3:
        while (true)
        {
            if (*f >= TJS_N('0') && *f <= TJS_N('9'))
                width = width * 10 + (*f - TJS_N('0'));
            else
                break;
            f++;
        }
    ...
    prec:
        if (*f == TJS_N('.'))
        {
            ...
            do
            {
                prec = prec * 10 + (*f - TJS_N('0'));
                f++;
            } while (*f >= TJS_N('0') && *f <= TJS_N('9'));
        }
            ...
            case TJS_N('s'):
            {
                ...
                if (str)
                {
                    ...
                    if (s + width > allocsize)
                    {
                        ...
                        {
                            tjs_uint inc_size;
                            ret->AppendBuffer(inc_size =
                                                  s + width - allocsize + TJS_VS_FS_OUT_INC_SIZE);
