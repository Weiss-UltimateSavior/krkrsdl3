	...
	const tTJSVariant &member)
{
	...
	if(member.Type() != tvtInteger && member.Type() != tvtReal)
	{
		...
		if(!name) TJSThrowFrom_tjs_error(TJS_E_MEMBERNOTFOUND, TJS_N(""));
		...
		else if(name[0] >= TJS_N('0') && name[0] <= TJS_N('9'))
		{
			...
			if(n == len) { *result = tTJSVariant(TJS_N("")); return; }
			if(n<0 || n>len)
				TJS_eTJSError(TJSRangeError);
            const tjs_char* ss = utf8_char_get(s, n);
            if (!ss) { *result = tTJSVariant(TJS_N("")); return; }
...
#define OBJ2 ((objthis)?(objthis):(Dispatch2))
	...
	tjs_error 	PropSetByNum(tjs_uint32 flag, tjs_int num, const tTJSVariant *param,
		iTJSDispatch2 *objthis)
	{
		tjs_error hr =
	...
	tjs_error 	CreateNewByNum(tjs_uint32 flag, tjs_int num, iTJSDispatch2 **result,
		tjs_int numparams, tTJSVariant **param,	iTJSDispatch2 *objthis)
	{
		tjs_error hr =
	...
	tjs_error 	IsInstanceOfByNum(tjs_uint32 flag, tjs_int num, const tjs_char *classname,
		iTJSDispatch2 *objthis)
	{
		tjs_error hr =
		...
		try
		{
                    ...
                    else
                    {
                        hr = clo.PropGet(flags, NULL, NULL, TJS_GET_VM_REG_ADDR(ra, code[1]),
                                         clo.ObjThis?clo.ObjThis:ra[-1].AsObjectNoAddRef());
                        if(TJS_FAILED(hr)) TJSThrowFrom_tjs_error(hr, NULL);
                    }
...
hr = clo.PropSet(flags,
			...
			if(TJS_FAILED(hr)) TJSThrowFrom_tjs_error(hr, *str);
						...
						hr = clo.PropSet(flags,
					NULL, NULL, TJS_GET_VM_REG_ADDR(ra, code[3]),
						clo.ObjThis?clo.ObjThis:ra[-1].AsObjectNoAddRef());
			if(TJS_FAILED(hr)) TJSThrowFrom_tjs_error(hr, NULL);
					}
			}
	...
	const tjs_int32 *code, tTJSVariant **args, tjs_int numargs)
{
	...
	if(TJS_FAILED(hr)) TJSThrowFrom_tjs_error(hr, TJS_N(""));
	...
	const ttstr & target, tTJSVariant **args, tjs_int numargs, tTJSVariant *result)
{
	...
	if(!member) TJSThrowFrom_tjs_error(TJS_E_MEMBERNOTFOUND, TJS_N(""));
...
#define TJS_STR_METHOD_IS(_name) (hash == StrFuncHash[TJSStrMethod_##_name] && !TJS_strcmp(StrFuncs[TJSStrMethod_##_name], member))
	...
	if(TJS_STR_METHOD_IS(charAt))
	{
		...
		if(result) *result = tTJSVariant(ttstr(bt, btLen));
		return;
	}
	else if(TJS_STR_METHOD_IS(indexOf))
	{
		...
		if(result) *result = utf8_indexOf(s, (const tjs_char*)*pstr, start);
		if(pstr) pstr->Release();
		return;
	}
		...
		if(result)
		{
			*result = s; // here s is copyed to *result ( not reference )
			...
			if(pstr)
			{
				...
				while(*p)
				{
					if(*p>=TJS_N('a') && *p<=TJS_N('z')) *p += TJS_N('Z')-TJS_N('z');
					p++;
				}
			}
		}
		...
		iTJSDispatch2 *objthis)
{
	...
	if(membername == NULL)
	{
		...
		case ctFunction:
		case ctExprFunction:
			if(!TJS_strcmp(classname, TJS_N("Function"))) return TJS_S_TRUE;
			break;

		case ctProperty:
			if(!TJS_strcmp(classname, TJS_N("Property"))) return TJS_S_TRUE;
			break;
			
		case ctClass:
			if(!TJS_strcmp(classname, TJS_N("Class"))) return TJS_S_TRUE;
			break;
		}
	}
...
//---------------------------------------------------------------------------
tjs_error 	tTJSInterCodeContext::GetCount(tjs_int *result, const tjs_char *membername,
...
//---------------------------------------------------------------------------
tjs_error 	tTJSInterCodeContext::DeleteMember(tjs_uint32 flag, const tjs_char *membername,
		tjs_uint32 *hint,  iTJSDispatch2 *objthis)
{
...
//---------------------------------------------------------------------------
tjs_error 	tTJSInterCodeContext::Invalidate(tjs_uint32 flag, const tjs_char *membername,
		tjs_uint32 *hint, iTJSDispatch2 *objthis)
{
...
//---------------------------------------------------------------------------
tjs_error 	tTJSInterCodeContext::IsValid(tjs_uint32 flag, const tjs_char *membername,
		tjs_uint32 *hint, iTJSDispatch2 *objthis)
{
...
//---------------------------------------------------------------------------
tjs_error 	tTJSInterCodeContext::Operation(tjs_uint32 flag, const tjs_char *membername,
