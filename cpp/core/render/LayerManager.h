...
public:
    ...
    //! @param		cy		触れている高さ
    //! @param		id		タッチ識別用ID
    virtual void NotifyTouchDown(
        tjs_real x, tjs_real y, tjs_real cx, tjs_real cy, tjs_uint32 id) = 0;
    ...
    //! @param		cy		触れている高さ
    //! @param		flag	タッチ状態フラグ
    virtual void NotifyTouchScaling(
    ...
    //! @param		cy		触れている高さ
    //! @param		flag	タッチ状態フラグ
    virtual void NotifyTouchRotate(tjs_real startangle,
                                   tjs_real curangle,
                                   tjs_real dist,
                                   tjs_real cx,
                                   tjs_real cy,
...
class tTVPLayerManager : public iTVPLayerManager, public tTVPDrawable
{
    ...
    tTVPLayerType
        DesiredLayerType; //!< desired layer type by the draw device for this layer manager
...
public:
    ...
    virtual void NotifyMouseDown(tjs_int x,
                                 tjs_int y,
                                 tTVPMouseButton mb,
    ...
    virtual void NotifyMouseOutOfWindow() override { MouseOutOfWindow(); }
    ...
    virtual void NotifyTouchDown(
        tjs_real x, tjs_real y, tjs_real cx, tjs_real cy, tjs_uint32 id) override
    {
        PrimaryTouchDown(x, y, cx, cy, id);
    }
    virtual void NotifyTouchUp(
        tjs_real x, tjs_real y, tjs_real cx, tjs_real cy, tjs_uint32 id) override
    {
    ...
    virtual void NotifyTouchScaling(
        tjs_real startdist, tjs_real curdist, tjs_real cx, tjs_real cy, tjs_int flag) override
    {
    ...
    virtual void NotifyMultiTouch() override { PrimaryMultiTouch(); }
